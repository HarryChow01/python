...
def print_info(response, para_dict):
    ...
    if 'items' in json_obj:
        ...
        while count < len(items):
            outfile.write(str(count + 1) + "\t\t\t" + items[count]['goods_id'] + "\t\t\t" + str(items[count]['score'])+ "\n")
            count += 1
    outfile.close()
...
def get_result(paras):
    ...
    url = "http://%s:%s/shard/mtsearch" % (host, port)
    ...
    param_str = ('"reqId":"%s", "userId":%d, "ip":"%s", "q":"%s", '
                '"size":%d, "sort":"%s", "cat":"%s", "cateNumShow":%d, '
                '"predictCats": %s, "version":"%s", '
                '"abFlag":"%s"    ') % \
                (reqId, userId, ip, query, size, sort, cat, cateNumShow,\
                predictCats, version, abFlag)

    print("param_str: " + param_str + "\n")

    param  = {"parameters": param_str}
